...
def splitDataset():
    ...
    pcd_filenames = glob.glob('data/objects_pcd/**/*.pcd', recursive=True)


    # Check if dataset data exists
    if len(pcd_filenames) < 1:
    ...
    train_validation_filenames = [filename for filename in pcd_filenames for obj in train_validation_objects if f'/{obj}/' in filename]
    test_filenames = [filename for filename in pcd_filenames for obj in test_objects if f'/{obj}/' in filename]
    ...
    # Print results
    print(f'Total point clouds: {len(pcd_filenames)}')
